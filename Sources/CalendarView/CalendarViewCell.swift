...
final class CalendarViewCell: UICollectionViewCell {
    ...
    var textColor: UIColor = UIColor.CalendarViewColor.black {
        didSet {
    ...
    var lineViewAppearance: CalendarView.LineView? {
        didSet {
            configureLineView()
        }
    }
    ...
    func setContentFont(size: CGFloat) {
...
private extension CalendarViewCell {
    ...
    func configureCircularView() {
        let smallerSize = bounds.width > bounds.height ? bounds.height : bounds.width
        let diameter = smallerSize * circularViewDiameter
