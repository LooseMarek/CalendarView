...
final class DateModel: NSObject {
    ...
    // Week text
    var weeks: (String, String, String, String, String, String, String) = ("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun")
    ...
    func display(in month: MonthType) {
        currentDates = []
        currentDate = month == .current ? dateNow : date(of: month)
        setup()
    }
    ...
    func date(of month: MonthType) -> Date {
        ...
        return calendar.date(byAdding: components, to: currentDate) ?? dateNow
    }
}
...
private extension DateModel {
    ...
    func setup() {
        var previouslySelectedDates: [Date] = []
        
        for selectedDate in selectedDates {
            if (selectedDate.value) {
                previouslySelectedDates.append(selectedDate.key)
            }
        }
        
        selectedDates = [:]
        
        for previouslySelectedDate in previouslySelectedDates {
            selectedDates[previouslySelectedDate] = true
        }
        ...
        currentDates = (0..<DateModel.maxCellCount).compactMap { index in
            ...
            .map { (date: Date) in
                let isSelected = previouslySelectedDates.contains(date)
    ...
    func atBeginning(of month: MonthType) -> Date {
        ...
        components.day = 1
        return calendar.date(from: components) ?? dateNow
    }
}
